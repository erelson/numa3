...
#LOADER_SPEED_ON = -24 # clockwise
#LOADER_SPEED_ON = 54  # counterclockwise
LOADER_SPEED_ON = -54  # counterclockwise; I swapped INB and INA
LOADER_SPEED_OFF = 0
...
GUNS_FIRING_DURATION = 80000 # us; 1/4 s

PARAM_LASER_PIN = Pin.board.X12
CMDR_ALIVE_CNT = 30 # ~750 ms
...
class NumaMain(object):
    ...
    def __init__(self,
        ...
        self.loader_timeout_mode = "timeout" # [running, timeout, rewind]
        self.loader_timeout_end = 3*LOADER_TIMEOUT_DURATION + ticks_us() # Adding the duration delay wasn't validated...
        self.loader_reverse_end = 0

        #self.loopLengthList = [6500, 2900, 2300, 1800, 1600, 1450,
        #self.loopLengthList = [2900, 2300, 1800, 1600, 1450, 1300,
        #self.loopLengthList = [2900, 2300, 1800, 1500, 1250, 1100,
        #self.loopLengthList = [2900, 2000, 1500, 1300, 1050, 750, # OK!
        self.loopLengthList = [2900, 2000, 1400, 1050, 820, 650, # Jerky at 19500!
        #self.loopLengthList = [2900, 2000, 1400, 1050, 800, 550, # Jerky at 19500!
        ...
        # Original bit one that spins wheel clockwise in loader
        self.ammoMotor = MotorDriver(Pin.board.Y9, Pin.board.Y12, Pin.board.Y10, cs=Pin.board.Y11)
        # New smaller one that spins wheel counter-clockwise in loader
        #self.ammoMotor = MotorDriver(Pin.board.Y12, Pin.board.Y9, Pin.board.Y10, cs=Pin.board.Y11)
        self.bb_detect_adc = ADC(Pin.board.X1)
        self.bb_detect_led = Pin(Pin.board.X2, mode=Pin.OUT)
        self.bb_detect_adc_loopcnt = 0
        self.MIN_ADC_NO_BB = 80#8
        ...
        # Defaults?
        trav_rate_default = 25 # TODO distance covered by steps in mm? or maximum deviation? A stride is twice this?
    ...
    #            #print("off")
        ...
        if self.gunbutton:
            ...
            self.loader_timeout_end = loopStart + LOADER_TIMEOUT_DURATION # microseconds; wait, what is this?

        # We check "panic" before anything else that might move the legs
        if self.crouch:
            self.zero_all_actions()
            #return 15000  # microseconds  #TODO why? 4-28
        ...
        if self.turnleft or self.turnright:
            ...
            # Make sure we continue turning for at least... 20 loops (todo what is a loop?
...
#            print("")
    ...
    def CmdrReadMsgs(self):
        ...
        if self.slowturret:
            #pan_add = int(-self.crx.lookh / 25)#17) # 17 is old value from Numa1
    ...
    def bb_loader_service(self, loopStart, adcval_effort, adcval_loaded):
        ...
        if not self.cmdrAlive or not self.enable_bb_loader:
            ...
            print("waiting2...")
        #elif adcval_loaded < 18 and self.loader_timeout_mode != "rewind":  # Only check this in forward dir
