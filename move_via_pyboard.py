...
def main():
    ...
    target.add_argument("-a", "--angle", type=float, metavar="DEG",
                        help="degrees from the servo's center position")
    ...
    if not (args.read or args.release or args.position is not None
            or args.angle is not None):
        ap.error("one of -p/-a/-r/--release is required")
    ...
    if args.release and pos is not None:
        how += ", then release"
    print("target: {} servo {} -> {}".format(bus, sid, how))
    ...
    if pos is None:
        action = "Release (stop holding) {} servo {}".format(bus, sid)
    elif args.release:
        action = "Move {} servo {} to {}, then release".format(bus, sid, pos)
    else:
        action = "Move {} servo {} to {}".format(bus, sid, pos)
