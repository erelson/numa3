...
class Bus:
    ...
    def fill_and_write_packet(self, dev_id, cmd, data=None):
        ...
        pkt_bytes[0] = 0x55  #0xff
        pkt_bytes[1] = 0x55  #0xff
        pkt_bytes[2] = dev_id
    ...
    def read_status_packet(self):
        ...
        # hiwonder just repeats the cmd byte in response, rather than supplying error bits
        #err = pkt.error_code()
        #if err != packet.ErrorCode.NONE:
        #    raise BusError(err)
        return pkt
    ...
    def send_read(self, dev_id, cmd):
        ...
        if self.show & Bus.SHOW_COMMANDS:
            ...
            log('Sending READ to ID {} CMD 0x{:02x}'.format(
                dev_id, cmd))
        self.fill_and_write_packet(dev_id, cmd)#, bytearray((offset, num_bytes)))
        # for reference:
        #fill_and_write_packet(self, dev_id, cmd, data=None):

    #def send_read_temperature(self, dev_id, offset):
    ...
    #        log('Sending READ to ID {} offset 0x{:02x} len {}'.format(
    #            dev_id, offset, num_bytes))
    #    self.fill_and_write_packet(dev_id, packet.Command.TEMP_READ)
    #    # for reference:
    #    #fill_and_write_packet(self, dev_id, cmd, data=None):
    ...
    def send_write(self, dev_id, cmd, data):#, deferred=False):
        if self.show & Bus.SHOW_COMMANDS:
    ...
    def write(self, dev_id, cmd, data):
        ...
        if dev_id == packet.Id.BROADCAST:
            return packet.ErrorCode.NONE
        # hiwonder appears to not acknowledge writes - we get `BusError: Rcvd Status: Timeout`
        #pkt = self.read_status_packet()
        return #pkt.error_code()
